...
void adc_init(void)
{
	...
	//使能ADC1工作
	//ADC_InitStructure.ADC_Resolution = ADC_Resolution_12b、10b、8b、6b;//使用最高精度进行测量
	...
	//告诉ADC1转换哪个通道，当前是转换通道6，顺序号为1（如果有多个通道进行扫描，这个数值决定扫描的顺序），测量采样的点的时间为3*(1/42MHz)
