...
void tim1_init(void)
{
	...
	//TIM1的硬件时钟频率=168000000/16800=10000Hz
	//只要进行10000次计数，就是1秒时间的到达，也就是1Hz的频率输出
	//只要进行10000/100次计数，就是1秒/100时间的到达，也就是100Hz的频率输出
	TIM_TimeBaseStructure.TIM_Period = 10000/200-1;//计数值100-1，决定了输出频率为100Hz
	TIM_TimeBaseStructure.TIM_Prescaler = 16800-1;//预分频，也就是第一次分频，当前16800分频，就是168MHz/16800
...
int main(void )
{
	...
	//初始化对应端口的引脚 
	GPIO_InitStructure.GPIO_Pin  = GPIO_Pin_13;//指定第13根引脚 
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF ;//配置为复用模式
	GPIO_InitStructure.GPIO_Speed =GPIO_Speed_50MHz ;//配置引脚的响应时间=1/100MHz .
	//从高电平切换到低电平1/100MHz,速度越快，功耗会越高
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP ;//推挽的输出模式，增加输出电流和灌电流的能力
	...
	while(1)
	{
		for(pwm_cmp=0; pwm_cmp<=50; pwm_cmp++)
		{
			//设置TIM14的通道1比较值,亮度递减
		...
		for(pwm_cmp=50; pwm_cmp>=0; pwm_cmp--)
		{
			//设置TIM14的通道1比较值,亮度递增
			TIM_SetCompare1(TIM14,pwm_cmp);
